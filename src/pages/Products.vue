<template>
  <div>
    <h1>Products</h1>
    <div class="products">
      <div v-for="(product, index) in products" :key="index" class="product-card"><!--displays all products using a loop -->
        <h3>{{ product.name }}</h3>
        <div class="cost">{{ product.price }}</div>

        <div class="product-image" :style="{ backgroundImage: `url(${product.image})` }"></div><!--displays images which are imported -->
...
export default {
  data() {
    return {
      products: [
        {  // array of product name a price and images
          name: "Guinness",
          price: "€5.10",
          image: guinness,
        },
  ...
  methods: {
    generateCustomerKey() {
      const min = 100; //min value for a 3-digit number
      const max = 999; //max value for a 3-digit number
      this.customerKey = String(Math.floor(Math.random() * (max - min + 1)) + min); //generates a random number between min and max
    },
    addItemToCart(product) { //adds product to cart using javascript
      const user = getAuth().currentUser;
      if (user) {
        const customerKey = this.customerKey; //use the same customerKey for all items
        //emits the event to add the product to the cart along with the customer key
